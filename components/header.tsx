...
export function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[var(--background)] border-b border-[var(--border)] shadow-md">
        ...
        <nav className="hidden lg:flex items-center gap-8">
          ...
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-sm font-medium tracking-wide text-[var(--foreground)] uppercase hover:text-[var(--primary)] transition-colors"
            >
              {link.label}
            </Link>
          ))}
          ...
          <Link
            href="/contact"
            className="ml-2 rounded-md bg-[var(--primary)] px-5 py-2.5 text-sm font-semibold text-white uppercase hover:opacity-90 transition"
          >
            Request a Quote
          </Link>
          ...
          <nav className="flex flex-col px-6 py-4">
            ...
            {navLinks.map((link) => (
              <Link
                ...
                href={link.href}
                onClick={() => setMobileOpen(false)}
                className="py-3 text-sm font-medium uppercase text-[var(--foreground)] border-b border-[var(--border)] hover:text-[var(--primary)]"
              >
                {link.label}
              </Link>
            ))}
            ...
            <Link
              href="/#contact"
              onClick={() => setMobileOpen(false)}
              className="mt-4 rounded-md bg-[var(--primary)] px-5 py-3 text-sm font-semibold text-white uppercase text-center"
            >
              Request a Quote
            </Link>
